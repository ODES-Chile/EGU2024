...
-   Resulting in two raster maps:\
    1) values of the `time scales` that `reached` the `maximum correlation` per drought index and\
...
# Results: Trend in zcNDVI {background-color='lightgrey'}
...
**Trends**
  ...
  -   The magnitude of the `trends` `increase/decrease` for `longer time scales`.
...
- We want to address the `relationship` between the drought trend (demand, supply, and soil moisture) and `land cover changes`.
